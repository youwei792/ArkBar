...
> **时点产物（2026-09-23 归档）**：本报告的结论对应提交 `68e4a1a`（2026-09-04），行号与统计已随后续改动漂移，
> 请按符号而非行号定位。**2026-09-24/25 的修复明细与当前验证状态见 `docs/handoff-2026-09-25.md`。**
> 以下是自基线以来**已核实**的变化，其余条目状态未重新审计：
>
> - **状态已变化（2026-09-24 复核）**：F-01（提交 `eb0f096` + 端点安全测试）、F-04（`0d21762` + 子进程环境隔离测试）、
>   F-18（`9fdc1f3` fail-closed 打包 + `69ce758` logo 数量校验）、F-02 的防抖部分（`853600a`）已修复。
> - **2026-09-24 第二轮修复**（见 `CHANGELOG.md` Unreleased 与对应提交）：F-08（LongCat 选择器接入）、
>   F-09 核心（Kimi 跨账号 token 配对）、F-10（Nebula userId 绑定同 profile）、F-12（错误正文截断，
>   日志脱敏测试仍缺）、F-15（概览口径统一）、F-16 同类（LongCat/StepFun/Nebula 导入假成功）、
>   F-17（钥匙串提示中性化，未做取消闸门）、F-19（CI release 门禁）、F-23（L10n 加锁）、
>   F-24（SECURITY.md/README 失实声明）、F-06（CredentialFileCache 并发）、StepFun 会话轮换回写。
> - **仍未处理**：F-03（统一重定向策略与响应上限）、F-05（失败传播细节）、F-07（无 generation/取消，
>   已加 pending 重刷缓解）、F-11（部分语义）、F-13（响应大小上限）、F-14（chat 兜底探测仍在，
>   已降为单次）、F-20（关键风险路径测试）、F-21、F-22（Nebula 分页截断）。
> - **§7 目标架构的 `ProviderStateStore` 已落地**：`UsageStore` 改为按 `ProviderTab` 键控的 Track 模型 +
>   单一 `[ProviderTab: TrackState]`（`UsageStore.swift`：审计基线 `68e4a1a` 时 860 行，
>   重构前 1140 行，现 619 行）。但报告要求的 **Task 句柄 / generation / 取消仍未实现**（代码里 0 处），
>   所以 F-07 依旧成立（2026-09-24 已加 pending 重刷缓解，未做取消）。
> - **测试数已变化**：约 100 → 164 项（2026-09-24 复核实测 164 项全绿，另新增本轮修复的回归测试）；
>   新增本地化键完整性遍历与语言固定断言（后者正是 CI 与本地宿主语言不一致时暴露出来的，属于 F-20/F-21 说的那类问题）。
> - **两处认证/上游依赖的正确性问题已修**：SenseNova 会话导入曾把统计 cookie 当作登录态接受（F-16 同一类的
>   "凭据来源不正确却报告成功"）；OpenCode 的 org id 曾依赖前端构建哈希，现改用控制台 JSON 接口。
> - **报告未覆盖、事后发现并修复的一个高影响问题**：菜单栏图标的 drawing-handler + 状态项快照构成自持续
>   重绘循环（实测常驻 ~64% 单核 CPU、每个刷新周期泄漏约 90MB）。静态审阅看不到这类问题，
>   说明 §6「本环境未执行」一节的限制是实质性的：任何性能/内存/运行时结论都需要真实 macOS 验证。
> - **两处认证/上游依赖的正确性问题已修**：SenseNova 会话导入曾把统计 cookie 当作登录态接受（F-16 同一类的
>   "凭据来源不正确却报告成功"）；OpenCode 的 org id 曾依赖前端构建哈希，现改用控制台 JSON 接口。
> - **报告未覆盖、事后发现并修复的一个高影响问题**：菜单栏图标的 drawing-handler + 状态项快照构成自持续
>   重绘循环（实测常驻 ~64% 单核 CPU、每个刷新周期泄漏约 90MB）。静态审阅看不到这类问题，

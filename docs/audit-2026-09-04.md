...
> 请按符号而非行号定位。以下是自基线以来**已核实**的变化，其余条目状态未重新审计：
>
> - **仍全部未处理**：F-01、F-03、F-04、F-05、F-06、F-07、F-08、F-09、F-10、F-11、F-12、F-13、F-14、
>   F-15、F-17、F-18、F-19、F-21、F-22、F-23、F-24（含全部 P1 安全门禁项）。本报告仍是待办清单，不是已完成记录。
...
>   重构前 1140 行，现 603 行）。但报告要求的 **Task 句柄 / generation / 取消仍未实现**（代码里 0 处），
>   所以 F-07 依旧成立。
> - **测试数已变化**：约 100 → 158 项；新增本地化键完整性遍历与语言固定断言（后者正是 CI 与本地宿主语言
>   不一致时暴露出来的，属于 F-20/F-21 说的那类问题）。

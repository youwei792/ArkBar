...
配额重置时间不等于套餐到期日。只有数据源提供经过验证的订单终止时间时，TokenBar 才显示到期徽标，不会用重置时间或本地凭据缓存猜测。当前已验证的来源：

- **火山方舟 Coding / Agent Plan**：签名 OpenAPI `ListSubscribeTrade`（AK/SK 与 arkcli SSO 两条链路都会查），取 `Status=Running` 订单的 `EndTime`，即控制台「我的订阅 → 结束时间」。团队版按席位计费，不会用个人订单的日期顶替。
- **智谱 Coding Plan**：`GET /api/biz/subscription/list`（复用已有的套餐 API Key，无需浏览器登录），取 `status=VALID` 订单的 `nextRenewTime`，即控制台「套餐概览 → 有效期至」；该字段缺失时回退到订单 `valid` 区间的结束日。
- **OpenCode Go / Kimi / LongCat / 阶跃 / 商汤 / 阿里云**：沿用各自接口里的续费日或过期字段。

到期查询是尽力而为的：接口失败只会隐藏徽标，额度圆环与菜单照常渲染。

到期时间也是「订阅到期提醒」的数据源：已接入套餐（火山与智谱的续费日、OpenCode Go 的续费日、LongCat 资源包到期时间等）自动进入提醒列表；未接入的订阅可在「设置 → 订阅提醒」手动添加（名称 + 到期日 + 备注），到期前按规则提醒。

...
- Added a GrokPool tab for the grok-farm gateway (new-api compatible): balance ring (cumulative spend vs spend + balance), today/monthly cost, token counts, request counts, and a cache-read/uncached/output split. All endpoints (`api/user/self`, `api/log/self`) authenticate with the same gateway API key as `/v1` model calls (`Authorization: Bearer`), stored in Keychain + file cache with a `GROKPOOL_API_KEY` environment fallback; the base URL defaults to `https://grok.axonlume.com`. Quota converts at the new-api default 500000 = $1, the tab is fully isolated from APINebula, and its settings pane offers the menu-bar percent-vs-balance choice (`$`).
